...
# ── Force UTF-8 on stdout/stderr (Windows + non-UTF-8 terminals like Git Bash) ──
...
def main() -> int:
    ...
    ap.add_argument(
        "--whisper",
        choices=["groq", "openai", "custom", "local"],
        default=None,
        help="Force a specific Whisper backend. Default: prefer custom, then Groq, fall back to OpenAI.",
    )
    ap.add_argument(
        "--no-dedup",
        action="store_true",
    ...
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        ...
        if backend:
            ...
            hint = (
                ...
                if args.whisper in ("groq", "openai", "custom") else
                f"--whisper {args.whisper} was set but faster-whisper is not installed or failed"
    ...
    # Rough estimation: audio ~2.1 MB / 2784 kB, each segment ~20 tokens
    whisper_segments = len(transcript_segments)
    estimated_transcript_tokens = whisper_segments * 20
    # Image tokens: ~512x528 res at ~1000 tokens per image
    frame_count = len(frames)
    estimated_image_tokens = frame_count * 1000
    estimated_total_tokens = estimated_transcript_tokens + estimated_image_tokens
    print(f"- **Total time:** {total_sec:.1f}s ({total_sec / 60:.1f} min)")
    print(f"- **Whisper model:** faster-whisper (small, local CPU)")
    print(f"- **Frames extracted:** {frame_count}")
    print(f"- **Transcript segments:** {whisper_segments}")
    print(f"- **Estimated transcript tokens:** ~{estimated_transcript_tokens}")
    print(f"- **Estimated image tokens:** ~{estimated_image_tokens} (at ~1k tok/img)")
