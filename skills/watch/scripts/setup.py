...
# Choose ONE backend:
#
# 0. Local (recommended for China users) — offline faster-whisper on CPU:
#    WHISPER_BACKEND=local
#    WHISPER_MODEL=small                    # tiny / base / small / medium / large-v3
#
# 1. Custom — any OpenAI-compatible ASR endpoint:
#    - 阿里云 DashScope:  WHISPER_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode
#                         WHISPER_MODEL=sensevoice-v1
#    - 百度千帆 / 腾讯云 / 火山引擎 etc.
#    - Self-hosted whisper-asr-server, Ollama whisper, etc.
WHISPER_BACKEND=
WHISPER_BASE_URL=
WHISPER_API_KEY=
WHISPER_MODEL=whisper-1
#
# 2. Groq (free tier, fast):      https://console.groq.com/keys
GROQ_API_KEY=
#
...
def _have_api_key() -> tuple[bool, str | None]:
    if _read_env_key("WHISPER_BACKEND") == "local":
        return True, "local"
    if _read_env_key("WHISPER_API_KEY"):
        return True, "custom"
...
def cmd_install() -> int:
    ...
    print("    WHISPER_BASE_URL=...   + WHISPER_API_KEY=...  (custom — any OpenAI-compatible ASR)")
    print("    GROQ_API_KEY=...       (Groq Whisper — console.groq.com/keys)")
    print("    OPENAI_API_KEY=...     (OpenAI Whisper — platform.openai.com/api-keys)")
    print("")
