...
def _find_ffmpeg() -> str:
    """Locate ffmpeg/ffprobe — including the one packaged by the `imageio-ffmpeg` wheel."""
...
# Output format for extracted frames (PNG for maximum compatibility)
FRAME_EXT = ".png"

def _frame_glob() -> str:
    return f"frame_*{FRAME_EXT}"

def _cue_glob() -> str:
    return f"cue_*{FRAME_EXT}"

def _frame_pattern(out_dir: Path, prefix: str = "frame") -> str:
    return str(out_dir / f"{prefix}_%04d{FRAME_EXT}")
...
def get_metadata(video_path: str) -> dict:
    """Probe video metadata. Uses PyAV when available (cross-platform, handles
    Unicode filenames), falls back to ffprobe."""
    # Always ensure ffmpeg/ffprobe are on PATH — other functions need them even
    # when metadata itself comes from PyAV.
    _find_ffmpeg()
    video_path_resolved = str(Path(video_path).resolve())

    # Try PyAV first — handles Unicode paths on Windows correctly
    try:
        import av  # noqa: E402

        container = av.open(video_path_resolved)
        streams = container.streams
        vid = next((s for s in streams if s.type == "video"), None)
        aud = next((s for s in streams if s.type == "audio"), None)
        duration = float(container.duration / av.time_base) if container.duration and container.duration > 0 else 0.0
        width = vid.width if vid else None
        height = vid.height if vid else None
        codec = vid.codec_context.name if vid and vid.codec_context else None
        meta = {
        ...
        # PyAV failed — fall back to ffprobe
        print(f"[watch] PyAV probe failed ({exc}), falling back to ffprobe…", file=sys.stderr)
...
def extract(
    ...
    for existing in out_dir.glob(_frame_glob()):
        existing.unlink()

    output_pattern = _frame_pattern(out_dir)
    cmd: list[str] = [
        "ffmpeg",
        "-hide_banner",
        "-loglevel", "error",
...
def extract_scene_candidates(
    ...
    """
    if shutil.which("ffmpeg") is None:
        raise SystemExit("ffmpeg is not installed. Install with: "
    ...
    for existing in out_dir.glob(_frame_glob()):
        existing.unlink()
    # ... after _require_ffmpeg in the other function — wait, need to check which function
...
def extract_at_timestamps(
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    for existing in out_dir.glob(_cue_glob()):
        existing.unlink()
    ...
    out: list[dict] = []
    for t in points:
        path = out_dir / f"cue_{len(out):04d}{FRAME_EXT}"
        cmd = [
            "ffmpeg",
            "-hide_banner",
            "-loglevel", "error",
...
def extract_keyframes(
    ...
    files = sorted(out_dir.glob(_frame_glob()))
