#!/usr/bin/env python3
"""Transcribe a video via Groq / OpenAI / custom Whisper-compatible API.
...
Pure stdlib — no SDK dependencies needed.
"""
...
def _read_custom_config() -> tuple[str, str, str]:
    """Read custom endpoint config from env or ~/.config/watch/.env.

    Returns (base_url, api_key, model). All empty strings if not configured.
    """
    dotenv_paths = [
            ...
            try:
                for line in path.read_text(encoding="utf-8").splitlines():
                    ...
                    if len(value) >= 2 and value[0] in ('"', "'") and value[-1] == value[0]:
                        value = value[1:-1]
                    # Strip inline comment (everything after first # preceded by a space)
                    import re as _re
    ...
    return (
        _lookup("WHISPER_BASE_URL", ""),
        _lookup("WHISPER_API_KEY", ""),
        _lookup("WHISPER_MODEL", "whisper-1"),
    )
...
def load_api_key(preferred: str | None = None) -> tuple[str, str] | tuple[None, None]:
    """Return (backend, api_key). Checks local first, then custom, then Groq, then OpenAI.

    If `preferred` is set, only that backend's key is considered.
    'local' returns ('local', '') — no API key needed.
    'custom' returns ('custom', api_key); the endpoint URL is NOT exposed here.
    """
    ...
    # Local backend: uses faster-whisper, no API key required
    # Enabled when WHISPER_BACKEND=local is set in env/dotenv,
    # OR when no other backend is configured (auto-detect by faster-whisper availability)
    if preferred is None or preferred == "local":
        # When explicitly requested (--whisper local), just check faster-whisper is installed
        if preferred == "local":
            try:
                import importlib
                ...
                raise SystemExit(
                    "Local Whisper backend requested but faster-whisper is not installed. "
                    "Install with: pip install faster-whisper"
                )
            return "local", ""

        # Auto-detect: check WHISPER_BACKEND=local first, then try faster-whisper
        # when no remote key is configured
        local_configured = _from_env("WHISPER_BACKEND") == "local"
        if not local_configured:
            for candidate in dotenv_paths:
                local_configured = _from_dotenv(candidate, "WHISPER_BACKEND") == "local"
                if local_configured:
                    break
        if local_configured:
            return "local", ""
        # Auto-detect: if faster-whisper is installed and no remote key exists
        try:
            import importlib
            importlib.import_module("faster_whisper")
        except ImportError:
            ...
            # Check that no remote backend key is configured
            remote_found = any(
                _from_env(k) or any(_from_dotenv(c, k) for c in dotenv_paths)
                for k in ("WHISPER_API_KEY", "GROQ_API_KEY", "OPENAI_API_KEY")
            )
            if not remote_found:
                return "local", ""

    # Custom backend: WHISPER_BASE_URL + WHISPER_API_KEY both required
    candidates = (
        ("WHISPER_API_KEY", "custom"),
        ("GROQ_API_KEY", "groq"),
        ...
        if not value:
            for candidate in dotenv_paths:
                ...
                if value:
                    break
        if value:
            # For custom, also verify base_url is set
            if backend == "custom":
                base_url = _from_env("WHISPER_BASE_URL")
                if not base_url:
                    for candidate in dotenv_paths:
                        base_url = _from_dotenv(candidate, "WHISPER_BASE_URL")
                        if base_url:
                            break
                if not base_url:
                    continue  # custom is not fully configured, fall through
...
def _prepare_audio(audio_path: Path) -> Path:
    """Ensure a mono 16kHz 64kbps mp3 file exists for the given audio path."""
    audio_path.parent.mkdir(parents=True, exist_ok=True)
    return audio_path


def _find_ffmpeg() -> str:
    """Locate ffmpeg (and ffprobe) — including the one packaged by the `imageio-ffmpeg` Python
    wheel when neither PATH nor /tmp has a native install."""
    for name in ("ffmpeg", "ffprobe"):
        if shutil.which(name):
            continue
        # Try standard locations first, including Windows %TEMP% (where Git Bash
        # /tmp maps to), then fall back to the imageio_ffmpeg bundled binary.
        tmp = os.environ.get("TEMP") or os.environ.get("TMP") or ""
        search_dirs = [tmp, "/tmp"] if tmp else ["/tmp"]
        for d in search_dirs:
            ...
            # Not found — try to copy from imageio_ffmpeg
            try:
                ...
                if os.path.isfile(src):
                    target_dir = tmp or "/tmp"
                    dst = os.path.join(target_dir, f"{name}.exe" if os.name == "nt" else name)
...
def _require_ffmpeg() -> None:
    """Raise SystemExit if ffmpeg/ffprobe are not usable."""
    _find_ffmpeg()
    if shutil.which("ffmpeg") is None:
        raise SystemExit(
            "ffmpeg is not installed. Install with:\n"
            "  pip install imageio-ffmpeg          (portable, no system install)\n"
...
def _resolve_endpoint(base_url: str) -> str:
    """Build the full /v1/audio/transcriptions endpoint from a base URL.
...
def _post_whisper(endpoint: str, api_key: str, model: str, audio_path: Path) -> dict:
    """Upload audio to a Whisper-compatible endpoint. Returns parsed JSON."""
    fields = {
        "model": model,
...
# ---------------------------------------------------------------------------
# Custom / self-hosted Whisper-compatible ASR backend
# ---------------------------------------------------------------------------

def _transcribe_local(audio_path: Path) -> list[dict]:
    """Transcribe via local faster-whisper (CPU, no network required).

    Configured via:
      WHISPER_MODEL — model size (default: small). Options: tiny, base, small, medium, large-v3
    """
    # 国内网络 — 使用 HF 镜像并禁用 Xet 传输协议
    os.environ.setdefault("HF_ENDPOINT", "https://hf-mirror.com")
    os.environ.setdefault("HF_HUB_DISABLE_XET", "1")
    _, _, model_size = _read_custom_config()
    model_size = model_size if model_size and model_size != "whisper-1" else "small"

    print(
        f"[watch] loading local faster-whisper model '{model_size}' (CPU/int8)…",
        file=sys.stderr,
    )
    from faster_whisper import WhisperModel  # noqa: E402

    model = WhisperModel(model_size, device="cpu", compute_type="int8")
    print(f"[watch] transcribing via local faster-whisper ({model_size})…", file=sys.stderr)

    segments_result: list[dict] = []
    segments, info = model.transcribe(str(audio_path), beam_size=5, language=None)
    print(
        f"[watch] detected language: {info.language} (probability: {info.language_probability:.2f})",
        file=sys.stderr,
    )
    ...
    print(f"[watch] local whisper: {len(segments_result)} segments", file=sys.stderr)
    return segments_result


def _transcribe_custom(api_key: str, audio_path: Path) -> list[dict]:
    """Transcribe via a user-configured Whisper-compatible endpoint.

    Configured via:
      WHISPER_BASE_URL — the base URL of the ASR service
      WHISPER_API_KEY  — the API key / token
      WHISPER_MODEL    — model name (default: whisper-1)

    Compatible with:
    - 阿里云 DashScope (compatible-mode): https://dashscope.aliyuncs.com/compatible-mode
    - 百度千帆 / 腾讯云 / etc. with OpenAI-compatible ASR endpoints
    - Self-hosted whisper-asr-server / faster-whisper-server
    - Ollama with whisper model
    """
    base_url, _, model = _read_custom_config()
    endpoint = _resolve_endpoint(base_url)

    print(
        f"[watch] uploading audio to custom endpoint ({endpoint}, model={model})…",
        file=sys.stderr,
    )

    return _segments_from_response(_post_whisper(endpoint, api_key, model, audio_path))
...
def _transcribe_file(backend: str, api_key: str, audio_path: Path) -> list[dict]:
    ...
    try:
        if backend == "groq":
            ...
            return _transcribe_custom(api_key, audio_path)
        elif backend == "local":
        ...
        raise SystemExit(f"Whisper {backend} backend failed: {exc}")
...
def transcribe_video(
    ...
    if not backend or (backend != "local" and not api_key):
        ...
        raise SystemExit(
            "No Whisper API key available. Set WHISPER_BACKEND=local (local faster-whisper), "
            "WHISPER_BASE_URL + WHISPER_API_KEY (custom), "
            "GROQ_API_KEY (Groq), or OPENAI_API_KEY (OpenAI) "
    ...
    def transcribe_one(path: Path) -> list[dict]:
        return _transcribe_file(backend, api_key, path)  # type: ignore[arg-type]
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("usage: whisper.py <video-path> [<audio-out.mp3>] [--backend groq|openai|custom]",
              file=sys.stderr)
        raise SystemExit(2)
